...
import Game.World.InWorldAreas.PokemonLeagueFirstRoom;
...
public class FightState extends InWorldState{
    ...
    public static boolean questTargetIsDead = false;
    public static boolean ethanIsDead = false;
    public static boolean redIsDead = false;
    ...
    private Animation playerThunderSkill;
    ...
    public FightState(Handler handler ,BaseHostileEntity enemy, String prevState) {
        ...
        enemyRect = new Rectangle((int) handler.getWidth() * 4/ 5 - 70,entityY - 200, 150, 300);
        ...
        playerThunderSkill = new Animation(20,Images.ThunderSkill);
    ...
    public void tick() {
        ...
        /////////////////////////////
            ...
            if(!attacking&&!defense&&!skill&&turn>0&&enemy.getHealth()<=0&&!battleOver){
                battleOver=true;
                // Condition to check if the quest target is dead or not.
                if (this.enemy.name.equalsIgnoreCase("Brock")) {
                    if (BaseNPC.questStarted) {
                        questTargetIsDead = true;
                    }

                }
                if (this.enemy.name.equalsIgnoreCase("Ethan")) {
                    ethanIsDead = true;

                }
                if (this.enemy.name.equalsIgnoreCase("Red")) {
                    redIsDead = true;
        ...
        /*
         * Add Debug key that turns on and off Debug mode in the Fight state.
         */
        ...
        // Set player health and mana to max in fight state.
        if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_H)) {
            ...
            handler.getGame().getMusicHandler().playEffect("res/music/HealthMusic.mp3",1);
            handler.getGame().getMusicHandler().play();
        }
        ...
        if(battleOver){
            ...
            if(alpha>=254){
                if(handler.getEntityManager().getPlayer().getHealth()==0){
                    ...
                    // If player defeats first trainer; spawn him on the same room
                    if (prevState.equals("PokemonLeagueFirstRoom")) {
                        handler.setArea(InWorldState.currentArea.name);
                        handler.setXInWorldDisplacement(PokemonLeagueFirstRoom.playerPLFirstRoomXSpawn);// Sets player coordinates after Ethan fight
                        handler.setYInWorldDisplacement(PokemonLeagueFirstRoom.playerPLFirstRoomYSpawn);// Sets player coordinates after Ethan fight
                    ...
                    // If player defeats second trainer; go to the main menu.
                    if (prevState.equals("PokemonLeagueSecondRoom")) {
                        State.setState(handler.getGame().menuState);
    ...
    private void drawCharacterInfo(Graphics2D g2) {
        ...
        for(int i = 0; i < 2;i++) {
            if(i==1) {//enemy
                ...
                // Draws "None" when player has no skill in the fight state 
            	...
            	//Draws the skill awarded by the quest in the fight state
                g2.drawString("Skill: " + String.valueOf(handler.getEntityManager().getPlayer().getquestSkill()), entityInfoX[i] + 15, (handler.getHeight() * 4 / 5) + 120);
                g2.drawString("Mana Cost: " + "25 MP", entityInfoX[i] + 15, (handler.getHeight() * 4 / 5) + 140);                
                }
            }
        }
...
//        	choose options to the left
    ...
    private void setUiManager() {
        ...
        uiManager.addObjects(new UIImageButton(handler.getWidth() * 38/60 - 128/2, 5*handler.getHeight()/6, 128, 64, Images.Skill, new ClickListlener() {
            @Override
            public void onClick() {
            	if(!BaseNPC.questCompleted) {   //Prevents player from using skill until he gain a skill via quest.
            		skill = false;
            	}
            	else if (handler.getEntityManager().getPlayer().getMana()>=25) {//Allows player to use skill once he completes the quest
    ...
    //Sets the background &  music according to the previous state
    private void backgroundSelect(String prev) {

        if(prev.equals("None")) {
           background = Images.battleBackground[1];
            
           handler.getGame().getMusicHandler().set_changeMusic("res/music/FightStateMusic.mp3");
           handler.getGame().getMusicHandler().play();
           handler.getGame().getMusicHandler().setVolume(0.2);
        }
        ...
        else if (prev.equals("PokemonLeagueFirstRoom")) {
            background = Images.battleBackground[4];
            handler.getGame().getMusicHandler().set_changeMusic("res/music/PLNPCMusic.mp3");
            handler.getGame().getMusicHandler().play();
            handler.getGame().getMusicHandler().setVolume(0.2);
        }
        else if(prev.equals("PokemonLeagueSecondRoom")) {
            	background = Images.battleBackground[4];
                handler.getGame().getMusicHandler().set_changeMusic("res/music/PLChampionMusic.mp3");
                handler.getGame().getMusicHandler().play();
                handler.getGame().getMusicHandler().setVolume(0.2);
       }    	
    ...
    private void callSkill(Graphics g) {
        ...
        // Draw Thunder frames
        g.drawImage(playerThunderSkill.getCurrentFrame(),(handler.getWidth() * 4/ 5)-93,entityY-200,256,256,null);
        ...
        // Changed Index to Thunder skill Array length.
        if(playerThunderSkill.getIndex()>=23){
            endTurn=true;
            green= 255;
            red=95;
            blue=255;
    ...
    private void EcallSkill(Graphics g) {
        enemyFireSkill.tick();
        g.setColor( new Color(Math.max(0,red--), Math.max(0,green--),Math.max(0,blue--)));
        g.drawImage(Images.tint(Images.playerFighter,Math.max(0,red--),0,0),playerRect.x,playerRect.y,playerRect.width,playerRect.height,null);
