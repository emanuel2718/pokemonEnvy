    ...
    private String getStatValue(String s) {
	    ...
	    switch (s){
            ...
            case "Max Health":
                ...
                //Determines if the player has a skill or not
            case "Skill":
                if(BaseNPC.questCompleted) {
            	return String.valueOf(handler.getEntityManager().getPlayer().getquestSkill());//Sets the skill awarded by the quest
                }else {
                return String.valueOf(handler.getEntityManager().getPlayer().getSkill());//Sets the skill to "None"
                }
        }
        return "0";
    }
